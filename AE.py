...
class AutoEncoder(nn.Module):
    ...
    def score_samples(self, X, cuda=True):
        if cuda:
            # 2024.12.10 trustee-Graph.py报错后添加
            # 如果 X 是 DataFrame，先转换为 ndarray
...
def train_process(dataset, subset):
    ...
    # 标准化，将数据转换为 均值为0，标准差为1 的分布 按列
    normalizer = StandardScaler()
    normalizer.fit(X_train)
    # 将特征列名保存到文件中，以便在测试阶段使用
    # 假设你已经有了 X_train 作为 numpy.ndarray
    X_train = pd.DataFrame(X_train)
    # 如果 X_train 没有列名，可以在这里给它指定一个默认的列名
